...
struct LpaStarCell
{
    ...
    //~ int key[2];
	 ...
	 char type;
	 //----------------------
	     bool   in_open   = false;                                      // whether currently considered in OPEN
	     double lastEnqK0 = std::numeric_limits<double>::infinity();    // last enqueued key[0]
	     double lastEnqK1 = std::numeric_limits<double>::infinity();    // last enqueued key[1]
